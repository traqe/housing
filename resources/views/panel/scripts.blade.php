...
<script>
    function GetSelected(){
        var grid = document.getElementById("appNumbers");
    ...
    /*   function getCemeteryOwners() {
        var url = '/housing/getCemeteryOwners';
        $.ajax({
            url : url,
            method : "get",
            data : {url: url},
            ...
            success : function(data)
            {
                ...
                var developers = JSON.parse(data);
                console.log(data);
                var trHTML = '<select  id="owner" name="owner" class="form-control input-group-lg reg_name" required > <option selected disabled>Select Owner</option>'
                $.each(developers, function (i, item) {
                      trHTML += '<option value= '+item.owner_id+'>'+item.name +' - '+ item.address+'</option>';
                });
                $('#owner').replaceWith(trHTML);
            }
        });
    }
*/
    ...
    function stageupdate(id) {
        console.log('Stage id is ' + id)
        $('#stage_id').val(id)
    }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(obj, function(i, item) {
                    ...
                    //document.getElementById("SetName").innerHTML =  item.Class;
                    //document.getElementById("RowID").innerHTML =  item.RowID;
                    trHTML += '<tr><td>' + item.StudentID + '</td><td>' + item.Name + '</td><td>' + item.class + '</td><td>' + '@if (Auth::user()->hasAnyRole(['
                    Administrator ','
                    Activity - Admin ']))' + '<a class="pull-right" title="Remove Student" onclick="rmv(' + id + ' ,' + item.StudentID + ')"><i class="fa fa-trash"></i></a>' + '@endif' + '</td></tr>';
                });
