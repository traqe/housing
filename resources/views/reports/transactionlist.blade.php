    ...
    <table class="tableBordered" style="width: 100%; font-size: 0.8em;">
        ...
        <tr>
            <td colspan="8" style="text-align: center"><h4
                        style=" font-size:12pt; font-weight:normal">TRANSACTIONS LIST REPORT
                    FROM {{$from}} TO {{$to}} @if ($user != null) FOR {{strtoupper($user->username)}} @endif</h4></td>
        </tr>
    </table>
...
<div>
        ...
        @forelse($transactions as  $s)
            @if ($s->loan != null)
                <tr>
                    ...
                    <td>
                        <span style="font-size:9pt;">{{$s->loan->client->person->first_name.' '.$s->loan->client->person->last_name}}</span>
                    </td>
                    ...
                    <td > <span style="font-size:9pt;">{{$s->user->username}}</span></td>
                </tr>
            @endif
        @empty
        @endforelse
    ...
    <table width="100%" style="width: 100%; font-size: 0.8em;" border="0">
        <tr>
            ...
            <td width="30%" align="right"> {{$transactions->count()}}</td>
        </tr>
    </table>
    <br>
