...
<div align="center">
    <table class="tableBordered"  style="width: 100%; font-size: 0.8em;" >
        ...
        <tr>
            <td colspan="8" style="text-align: center"> <h4 style=" font-size:12pt; font-weight:normal" >NEW CLIENTS REPORT FROM {{$from}} TO {{$to}}</h4></td>
        </tr>
    </table>
...
<div>
    <table class="table"  style="width: 100%; font-size: 0.8em;" border="0">
        ...
        @forelse($clients as $key => $client)
            ...
            <tr>
                ...
                <td  align="center">{{$client->person->first_name}}<span style="font-size:9pt;"></span></td>
                <td > <span style="font-size:9pt;">{{$client->person->last_name}}</span></td>
                <td > <span style="font-size:9pt;">{{$client->person->nationalId}}</span></td>
                <td > <span style="font-size:9pt;">{{$client->clientNo}}</span></td>
                <td > <span style="font-size:9pt;">{{$client->person->mobile_no}}</span></td>
                <td > <span style="font-size:9pt;">{{$client->person->gender->gender}}</span></td>
                <td > <span style="font-size:9pt;">{{$client->person->date_of_birth}}</span></td>
                <td > <span style="font-size:9pt;">{{$client->created_at}}</span></td>
            </tr>
            {{--@endif--}}
        @empty
        @endforelse
    ...
    <table  style="width: 100%; font-size: 0.8em;" border="0">
        ...
        <tr>
            <td>Number of New FeMale Customers</td>
            <td>{{$female}}</td>
        </tr>
    </table>
