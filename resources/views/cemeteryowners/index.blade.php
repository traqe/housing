...
<div class="card card-accent-primary">
                ...
                <table class="table table-striped table-bordered table-hover"  id="example">
                        ...
                        <tbody>
                            @forelse($cemeteryowners as $owner)
                            <tr>
                                <td>{{$owner->id}}</td>
                                <td>{{$owner->name}}</td>
                                <td>{{$owner->surname}}</td>
                                <td>{{$owner->Identity_no}}</td>
                                <td>{{$owner->contact}}</td>
                                <td>{{$owner->address}}
                                    <div class="pull-right box-tools">
                                        <a class="text-warning"
                                           href="{{route('showCemeteryOwner',$owner->id )}}"
                                           title="View Details"><i class="fa fa-eye"></i>
                                        </a>
                                        <a  class="text-primary"
                                            href="{{route('editCemeteryOwner',$owner->id )}}"
                                            title="Edit Details"><i class="fa fa-pencil-square-o"></i>
                                        </a>
                                        {{--<a  class="text-danger"--}}
                                            {{--href="{{route('deleteCemeteryOwner',$owner->owner_id )}}"--}}
        ...
        <div class="modal fade" id="deleteC" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    ...
                    <form  class="form-horizontal" action="/owners/{{0}}" method="post">
